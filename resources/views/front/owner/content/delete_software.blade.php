<div class="col-12 col-sm-9 col-md-7 col-lg-6 col-xl-5 mt-5 mx-auto">
    <div class="alert alert-warning mx-auto text-center" role="alert">
        <form action="{{ route('owner_delete_software_result') }}" method="POST">
            @csrf
            <div><b><u>{{$user->name}}</u></b> Kullanıcısı Üzerindeki</br>
            <b><u>{{$delete->name}}</u></b> Türü: <b><u>{{$delete->type}}</u></b></br>
            Olan Yazılımı Silmek Üzeresiniz! </div>
            ...
            <input type="hidden" name="soft_id" value="{{$delete->id}}">
        </form>
    </div>
</div>
@if (Cookie::get('error'))
