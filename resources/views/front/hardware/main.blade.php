    ...
    <script>
        ...
        function hardwareDelete(id) {
            $.ajax({
                ...
                success: function(response) {
                    $('#hardware_delete_barcode_number_info').text(response.barcode_number);
                    $('#hardware_delete_barcode_number').val(response.barcode_number);
                    $('#hardware_delete_serial_number').text(response.serial_number);
                    $('#hardware_delete_detail').html(response.detail);
                    $('#hardware_delete_type').text(response.get_type.name);
                    $('#hardware_delete_model').text(response.get_model.name);
                }
            });
        }
        ...
        function hardwareUpdate(id) {
            $.ajax({
                ...
                data:{id},
                success: function(response) {
                    $('#hardware_update_barcode_number').val(response.barcode_number);
                    $('#hardware_update_barcode_number_info').val(response.barcode_number.slice(response.get_type.prefix.length));
                    $('#hardware_update_barcode_number_prepend').text(response.get_type.prefix);
                    $('#hardware_update_serial_number').val(response.serial_number);
                    $('#hardware_update_serial_number_info').val(response.serial_number);
                    //$('.hardware_update_type_select').select2("val", response.type_id);
                    $('.hardware_update_type_select').val(response.type_id).trigger('change');
                    //$('.hardware_update_model_select').select2("val", response.model_id);
                    $('.hardware_update_model_select').val(response.model_id).trigger('change');
                    $('#hardware_update_detail').html(response.detail);
                    $('#hardware_update_duration').val(response.duration);
                }
            });
        }
